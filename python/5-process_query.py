...
TOP_RESULTS = 5  # number of relevant chunks to retrieve
...
#   - ollama_cloud (minimax-m3:cloud) → works on any device with Ollama logged in, but slower
DEFAULT_BACKEND = "ollama_cloud"  # change to "lmstudio_2b" or "lmstudio_4b" if you have LM Studio running
...
def inference_ollama_cloud(prompt, model="minimax-m3:cloud"):
    """Get a response from Ollama cloud. Works on any device with ollama logged in."""
    r = requests.post(f"{OLLAMA_URL}/api/generate", json={
        "model": model,
        "prompt": prompt,
        "stream": False,
...
def inference_lmstudio_4b(prompt, model="google/gemma-4-e4b", temperature=0):
...
LLM_BACKENDS = {
    ...
    "lmstudio_4b": inference_lmstudio_4b,
    "lmstudio_2b": inference_lmstudio_2b,
    "ollama_cloud": inference_ollama_cloud
}
